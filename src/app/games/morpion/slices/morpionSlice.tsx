...
export const MorpionSlice = createSlice({
  ...
  reducers: {
    ...
    setGrille(
      state,
      action: PayloadAction<{ ligne: number; colonne: number }>
    ) {
      let newgrille = [...state.grille];
      newgrille[action.payload.ligne][action.payload.colonne] =
        state.statut === "player1" ? 1 : 2;
    },
...
export const selectPlayer1Points = (state: RootState) =>
  state.morpion.player1Point;
export const selectPlayer2Points = (state: RootState) =>
  state.morpion.player2Point;

          ...
          <>
            {ligne.map((box, key) => (
              <Row
                key={key}
                box={box}
                statutGame={statutGame}
                onClick={() => handleClick(keyLigne, key)}
              />
            ))}
          </>
        ))}
...
const Row = ({
  ...
  statutGame: MorpionInitialType["statut"];
  box: rowType;
  onClick: Function;
}) => {
  ...
  return (
    <div
      className={
        ...
        (isClick === 2 ? "deux" : "") +
        " " +
        (statutGame === "end" && winner === isClick ? "victory-case" : "")
      }
      onClick={(e) => handleClick()}
    >
      <SymbolePlayer player={box} />
    </div>
  );
};

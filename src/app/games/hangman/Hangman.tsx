...
const Hangman = () => {
  ...
  const clickOnLetter = (letter: string) => {
    if (wordsToFind) {
      ...
      cpWordsToFind = wordsToFind.map((wordToFind: IWord) => {
        ...
        return {
          ...wordToFind,
          lettersToFind: wordToFind.lettersToFind.replaceAll(letter, ""),
        };
      });
  ...
  return (
    <WindowGame
      ...
      gameName={listGames[2].name}
      desc={listGames[2].desc}
    >
      <AnimatePresence>
        {isLoose && (
          <motion.div
      ...
      <div className="selector-variant">
        ...
        {Object.keys(variantes).map((key, index) => (
          <div
            key={index}
            onClick={() => setVariant(key as AllowedVariants)}
            data-selected={key === variant}
          >
            {variantes[key as AllowedVariants].name}
          </div>
        ))}
      </div>
    </WindowGame>

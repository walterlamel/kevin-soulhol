...
const Ennemy = ({index, destroy} : {index: number, destroy: Function}) => {
    ...
    const [positionX, setPositionX] = useState<number>(Math.random() * (95 - 5) + 5);
    const [positionY, setPositionY] = useState<number>(optionsRunner.EnnemyInitialPositionY);
    const [isBonus, setIsBonus] = useState<boolean>(false);
    const [size, setSize] = useState<number>(Math.floor(Math.random() * (optionsRunner.EnnemyMaxSize - optionsRunner.EnnemyMinSize) + optionsRunner.EnnemyMinSize));
    const [speed, setSpeed] = useState<number>(optionsRunner.EnnemyinitialSpeed);
    const [colisionEnnemy, setColisionEnnemy] = useState<positionType>({left : positionX, right : 0, top : 0, bottom : 0});
    const [action, setAction] = useState<"fall" | "pause" | "destruct" | "touched">("fall");
    ...
    return (
        <div className={"ennemy " + (action === "touched" ? "touched" : "") + " " + (isBonus ? "bonus" : "")} ref={ref} style={{ top : positionY, left : positionX + "%", width : isBonus ? optionsRunner.sizeBonus : size}}></div>
    )
}

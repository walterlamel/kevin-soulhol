...
import { changeColisonPlayer, positionType, selectStatut } from "../slices/runnerSlice";
...
const Player = ( ) => {
    ...
    const [targetPositionTouch, setTargetPositionTouch] = useState<number|null>(null)
    ...
    function handleMoveMouse(e : MouseEvent){
        if(ScreenSize && WindowSize){
            ...
            percent = ((position - (margepercent/100*ScreenSize)) / (ScreenSize - (margepercent/100*ScreenSize))) * 100;
    ...
    return (
        <div className="space-drag">
            <motion.div className="player" ref={ref} style={{left : positionX}} ></motion.div>
        </div>
    )
}

...
const Works = () => {
  const [selectedFilterTool, setSelectedFilterTool] = useState<number | false>(
    false
  );
  ...
  return (
    <>
      ...
      <div className="work-page otherpage">
        ...
        <p>
          Entre programation, illustration et graphisme, il y a un peu de tout.
          Ce sont des projets sur lesquels j'ai travaillé, soit durant ma
          scolarité, soit pour des clients, soit même pour mon propre plaisir.
        </p>
        <div className="container-filter-tools">
          <div
            className={"filter-outil" + (!selectedFilterTool ? " select" : "")}
            onClick={() => handleClickFilterTool(false)}
          >
          ...
          {outils.map((o: outilType, k: number) => (
            <div
              className={
                "filter-outil" + (selectedFilterTool === o.id ? " select" : "")
              }
              key={k}
              onClick={() => handleClickFilterTool(o.id)}
            >
              <img
                src={

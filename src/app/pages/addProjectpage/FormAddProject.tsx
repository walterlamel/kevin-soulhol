...
export const FormAddProject = () => {
    ...
    if (result.data.errors) {
      ...
      result.data.errors.forEach(function (codeerror: any) {
        let inp = document.querySelector(
          'label[for="' + codeerror.source.pointer + '"]'
        );
        inp?.classList.add("error");
      });
    } else if (result && !result.res) {
      setErrorMsg(
        "Une erreur est survenue lors de l'enregistrement : " + result.message
      );
    } else {
      navigate("/");
    }
  ...
  async function handleClickAddImg() {
    if (allImg.length < 10) {
      ...
      setTimeout(() => {
        const lastcreated: HTMLInputElement | null = document.querySelector(
          "#image" + (index + 1)
        );
        if (lastcreated) {
          lastcreated.click();
  ...
  async function changeSelectedTools({
    add,
    supp,
  }: {
    add: outilType;
  ...
  return (
    <div className="pageFormAdd otherpage">
      <h2>
        <span className="accent">Ajouter</span>
        <br />
        un projet
      </h2>
      <form action="" onSubmit={(e) => handleSubmit(e)}>
        ...
        <label htmlFor="type">
          ...
          <input
            ...
            name="type"
            id=""
            defaultValue={project ? project.type : ""}
          />
        </label>
        ...
        <label htmlFor="short_desc">
          ...
          <input
            ...
            name="short_desc"
            id=""
            defaultValue={project ? project.short_desc : ""}
          />
        </label>
        ...
        <label htmlFor="in_homepage" className="checbox-label">
          ...
          <input
            ...
            name="in_homepage"
            id=""
            defaultChecked={project.in_homepage}
          />
        </label>
        ...
        <label htmlFor="is_brouillon" className="checbox-label">
          ...
          <input
            ...
            name="is_brouillon"
            id=""
            defaultChecked={project.is_brouillon}
          />
        </label>
        ...
        <label htmlFor="outils">
          ...
          <SelectTool
            ...
            usedTools={project.outils}
          />
        </label>

        <label htmlFor="is_brouillon">Image principale</label>
        ...
        >
          ...
          <input
            ...
            name="coverimage"
            id="coverimage"
            onChange={(e) => handleChangeCoverImg(e)}
          />
        </div>
        ...
        <div className="container-images">
          {allImg.map((n, k) => (
            <SelectImg
              ...
              index={n}
              img={
                project?.images && project?.images[n] ? project.images[n] : ""
              }
              deleting={() => handleDeleteImg(n)}
            />
          ))}
